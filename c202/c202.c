...
*/
	return (s->top==-1)?1:0; // -1 means empty stack
}
...
*/
	return (s->top==STACK_SIZE-1)?1:0; // STACK_SIZE-1 means full stack
}
...
** funkci stackEmpty.
*/
	if (stackEmpty(s)) { // pokud je zasobnik prazdny, chyba
		stackError(SERR_TOP);
		return;
	}
	*c = s->arr[s->top]; // navraceni hodnoty odkazem
}
...
//		stackError(SERR_TOP);
	...
	s->top--; // snizeni poctu znaku na zasobniku (respektive posunuti stack pointeru)
}
...
** funkci stackFull.
*/
	if (stackFull(s)) { // pokud je zasobnik plny, chyba
		stackError(SERR_PUSH);
		return;
	}
	s->top++; // posunuti stack pointeru
	s->arr[s->top] = c; // prirazeni znaku
}
